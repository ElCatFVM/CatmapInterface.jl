...
# ╔═╡ 48914d19-1656-4579-856f-661d563e4aec
# ╠═╡ disabled = true
#=╠═╡
begin
...
# ╔═╡ 2d520e2e-f6ef-448f-885f-f65932576826
begin
    pypath = "/home/bmchun/miniconda3/envs/byungmin/bin/python"
    if Sys.isexecutable(pypath)   # 여기 Sys. 붙여야 합니다!
        ENV["PYTHON"] = pypath
        Pkg.build("PyCall")
    end
end
...
# ╔═╡ e42c5df9-a98b-4ea0-86fa-76a00217491b
...
begin
	...
	barplot(tbl.cat, tbl.height,
	        axis = (xticks = (1:11, ["GH2OΔCOΔele_t", "GCHΔOHΔele_t", "GOCCOΔH2OΔele_t", "GCOOHΔH2OΔele_t",
             "GCOΔ_t", "GOCΔCO_t", "GCO2Δ_t",
             "βH2OΔCOΔele_t", "βCHΔOHΔele_t", "βOCCOΔH2OΔele_t", "βCOOHΔH2OΔele_t"]),
	                title = "Total Order Indices"),
	        )
end
  ╠═╡ =#

# ╔═╡ abb5def9-272a-4a05-b77b-36926c40f698
md"""
### Bayesian optimization
"""

# ╔═╡ d042ce7e-e324-4090-b386-7133a9631fc1
...
#=╠═╡
open("log_txt/opt_inter_domain_red_low_tol_with_iter=$(n_iter)_init=$(init_points)_new.txt", "a") do io
...
#=╠═╡
open("log/opt_inter_domain_red_low_tol_with_iter=$(n_iter)_init=$(init_points)_new.log.json", "a") do io
        println(io, "reltol: $(reltol), abstol:$(abstol)")
		println(io, "iter:$(n_iter), init:$(init_points)")
		println(io, "random_state:$(random_state)")
		println(io, "best_result:$(best_params)" )
...
# ╔═╡ b501193e-22ce-4651-9f35-0a441b7d22bb
catmap_params

# ╔═╡ 7d09c5ba-b748-4eab-b490-41580b96c359
best_result
...
#=╠═╡
begin
	logger = Logger.JSONLogger(path="../log/opt_expanded_large_tol_with_iter=$(n_iter)_init=$(init_points)_new.log")
	optimizer.subscribe(Eve.Events.OPTIMIZATION_STEP, logger)
end
  ╠═╡ =#
...
#=╠═╡
optimized=optimizer.max["params"]
  ╠═╡ =#

# ╔═╡ 9442f6b6-7139-426f-add8-88391adc92a0
#=╠═╡
best_parameters = (
    GH2OΔCOΔele_t      = optimized["GH2OΔCOΔele_t"],
    GCHΔOHΔele_t       = optimized["GCHΔOHΔele_t"],
    GOCCOΔH2OΔele_t    = optimized["GOCCOΔH2OΔele_t"],
    GCOOHΔH2OΔele_t    = optimized["GCOOHΔH2OΔele_t"],
    GCOΔ_t             = optimized["GCOΔ_t"],
    GOCΔCO_t           = optimized["GOCΔCO_t"],
    GCO2Δ_t            = optimized["GCO2Δ_t"],
    βH2OΔCOΔele_t      = optimized["βH2OΔCOΔele_t"],
    βCHΔOHΔele_t       = optimized["βCHΔOHΔele_t"],
    βOCCOΔH2OΔele_t    = optimized["βOCCOΔH2OΔele_t"],
    βCOOHΔH2OΔele_t    = optimized["βCOOHΔH2OΔele_t"]
)
...
# ╔═╡ 2e049f00-b373-4f8a-b129-a053aa04b4d9
#=╠═╡
loss_function(best_parameters)
  ╠═╡ =#
...
# ╔═╡ 5134439c-3ebd-47f5-9e61-896a568bea8c
#=╠═╡
global_sens = gsa(peak_cases, Sobol(), [(0.01, 2.0), (0.01, 2.0), (0.01, 2.0), (0.01, 2.0), (0.01, 1.0), (0.01, 1.0), (0.01, 1.0),(0.01, 1.0), (0.1, 0.9), (0.1, 0.9), (0.1, 0.9)]  ; samples = 1000)
  ╠═╡ =#

# ╔═╡ 455a4e7d-e1c5-4d33-b767-f7006bde60f7
#=╠═╡
global_sens
  ╠═╡ =#
