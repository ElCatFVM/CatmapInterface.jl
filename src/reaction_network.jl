...
If `conserve_pressures==true` (`false` by default),  conserve the pressures of the gaseous and fictious species involved in the heterogeneous reaction network.
...
If `symbolic_formation_energies==true` (`true` by default), make the formation energies symbolic parameters of the reaction network,
otherwise use just their numerical values.
"""
function create_reaction_network(catmap_params::CatmapParams;
    ...
    compute_free_energies!(free_energies, catmap_params::CatmapParams,
                           formation_energies, θ, σ, ϕ_we, ϕ, local_pH, β;
                           symbolic_formation_energies)
