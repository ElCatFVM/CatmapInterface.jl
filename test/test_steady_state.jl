...
using Catalyst: Catalyst, netstoichmat, numreactions, reactionrates, species,
    speciesmap, symmap_to_varmap, ode_model, complete
...
"""
ssolve(odesys, params::SSParams; solver=DynamicSS(Rodas5P()), maxiters=1e6)

Solve the `odesys` for the steady state with the parameter set in `params`. 
"""
function ssolve(odesys, params; solver = DynamicSS(Rodas5P()), maxiters = 1.0e6)
    (; u0, ps) = params
...
function test_steady_state_with_catmap(rn::Catalyst.ReactionSystem, odesys, catmap_params, template_file_path, ss_params; solver = DynamicSS(Rodas5P()), maxiters = 1.0e6, rtol = 1.0e-3)
    ...
    =#
    return @testset "species=$(string(species))" for (species, θcatmap) in catmap_ssol
...
"""
function runtests()
    return @testset "Stationary Solutions" begin
